...
int main() {
    ...
    for (int r = 0; r < M; r++)
        for (int c = 0; c < N; c++) {
            ...
            for (int k = 0; k < K; k++)
                C_cpu.elements[c * C_cpu.height + r] +=
                    A.elements[r + k * A.height] * B.elements[k + c * B.height];
            // C_cpu.elements[c + r * C_cpu.width] += A.elements[k + r * A.width] * B.elements[c + k
            // * B.width]; // Row-Major format
        }

    {
        ...
        cudaMemcpy(d_B.elements, B.elements, d_B.width * d_B.height * sizeof(float),
                   cudaMemcpyHostToDevice);
        cudaMemcpy(d_C.elements, C.elements, d_C.width * d_C.height * sizeof(float),
                   cudaMemcpyHostToDevice);
        ...
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, K, &alpha, d_A.elements, M,
                    d_B.elements, K, &beta, d_C.elements, M);
